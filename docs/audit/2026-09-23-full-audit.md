...
| C2 | **High** | The **vendored `third_party/webrtc-kmp` fork** (Apache-2.0) carries the upstream README unchanged, with no statement of modifications (Apache-2.0 §4(b) requires prominent notices in modified files) and no upstream NOTICE passed through. | Add a `MODIFICATIONS.md` and file headers; carry the upstream NOTICE. **Correction (Phase 5):** neither upstream repo (shepeliev, aschulz90) has a NOTICE file, so there is nothing to carry. |
| C3 | Medium | The root `NOTICE` is two lines and doesn't cover the bundled Apache components' NOTICE contents. | Aggregate them. **Correction (Phase 5):** a scan of every shipped jar/aar found one NOTICE (`jakarta.inject-api`), plus webrtc-java's repository NOTICE. This finding was smaller than stated. C1 also **missed** Skia and its bundled libraries inside skiko (desktop). |
